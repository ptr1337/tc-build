...
def parse_parameters(root_folder):
    ...
    parser.add_argument("-t",
                        ...
                        """),
                        type=str,
                        default="AArch64;ARM;BPF;M68k;Mips;PowerPC;RISCV;SystemZ;X86")
    # yapf: enable
...
def project_target_cmake_defines(args, dirs, stage):
    """
    ...
    if bootstrap_stage(args, stage):
        ...
        # M68k is experimental so it needs to have special handling
        if "M68k" in targets and dirs.root_folder.joinpath(
                "llvm-project/llvm/lib/Target/M68k").exists():
            experimental_targets = "M68k"
        targets = targets.replace("M68k", "")
    ...
    if experimental_targets:
        defines['LLVM_EXPERIMENTAL_TARGETS_TO_BUILD'] = experimental_targets
...
def build_cmake_defines(args, dirs, env_vars, stage):
    ...
    defines.update(project_target_cmake_defines(args, dirs, stage))
