...
def kernel_build_sh(args, config, dirs, profile_type):
    ...
    :param dirs: An instance of the Directories class with the paths to use
    :param profile_type: The type of profile we are building ("bolt-instrumentation", "bolt-sampling", or "pgo")
    :return:
    """
    ...
    if profile_type == "bolt-sampling":
        ...
        # We do not want to build too big of a perf profile, as perf2bolt calls
        # 'perf script', which can consume large amounts of memory. Just do a
        # native build if the host target is in the list of targets or the
        # first target if not.
...
def do_bolt(args, dirs):
    # Check if sampling can be used by calling perf if it exists; if not,
