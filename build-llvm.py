...
def project_cmake_defines(args, stage):
    ...
    # Validate that the user has "bolt" in the list of projects if they are doing a one stage build
    if args.bolt and args.build_stage1_only:
        if projects != "all" and "bolt" not in projects:
            utils.print_error(
                'BOLT requested but "bolt" is not in the list of projects: {}'.
                format(projects))
            exit(1)
...
def kernel_build_sh(args, config, dirs, profile_type):
    """
    Run kernel/build.sh to generate BOLT or PGO profiles
    :param args: The args variable generated by parse_parameters
    :param config: The config to build (defconfig, allmodconfig, allyesconfig)
    :param dirs: An instance of the Directories class with the paths to use
    :param profile_type: The type of profile we are building ("bolt" or "pgo")
    :return:
    """
    # Run kernel/build.sh
    build_sh = [
        dirs.root_folder.joinpath("kernel", "build.sh"),
        '--{}'.format(profile_type), '-t',
        get_targets(args)
    ]
    if profile_type == "bolt":
...
def generate_pgo_profiles(args, dirs):
    ...
    for pgo in args.pgo:
        if pgo.split("-")[0] == "kernel":
            kernel_build_sh(args, pgo.split("-")[1], dirs, "pgo")
        if pgo == "llvm":
...
def do_bolt(args, dirs):
    utils.print_header("Peforming BOLT")

    llvm_bolt = dirs.build_folder.joinpath("stage1", "bin", "llvm-bolt")

    # clang-#: original binary
    # clang.inst: instrumented binary
    # clang.bolt: BOLT optimized binary
    # .inst will be removed and .bolt will become original binary after optimizations
    ...
    clang_inst_cmd = [
        ...
        "--instrumentation-file={}".format(bolt_profile), "-o", clang_inst,
        clang
    ]
    show_command(args, clang_inst_cmd)
    subprocess.run(clang_inst_cmd, check=True)

    # Generate profile data by using instrumented clang to build kernels
    kernel_build_sh(args, "defconfig", dirs, "bolt")
    ...
    show_commmand(args, clang_opt_cmd)
...
def main():
    ...
    if args.bolt and platform.machine() != "x86_64":
        utils.print_error("\n--bolt is currently x86_64 only!")
        exit(1)
